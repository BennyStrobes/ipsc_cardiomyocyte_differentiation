...
def get_variant_gene_pairs(sig_genes_file):
    f = open(sig_genes_file)
    dicti = {}
    for line in f:
        line = line.rstrip()
        data = line.split()
...
def make_all_tests_comparison_file(all_hits_file, sig_file, time_step_independent_stem, dynamic_egenes_comparison_file, model_version, threshold, pvalue_mapping):
    # Extract number of significant genes
    #num_genes = extract_number_of_genes(sig_egene_file)
    variant_gene_pairs = get_variant_gene_pairs(sig_file)
