...
run_simulation <- function(maf, num_simulations, num_individuals, num_time_steps, interaction_effect_size, sdev, linear_effects_sdev, output_file) {
	sink(output_file)
	...
	for (interaction_effect_size_iter in 1:length(interaction_effect_size_arr)) {
		for (sdev_iter in 1:length(sdev_arr)) {
			for (linear_effects_sdev_iter in 1:length(linear_effects_sdev_arr)) {
				...
				counter <- counter + 1
				output_file <- paste0(power_analysis_dir, "simulation_results_", num_individuals, "_individuals_", interaction_effect_size, "_interaction_beta_", sdev, "_sdev_", linear_effects_sdev, "_linear_effects_sd_", num_simulations,"_simulations_", num_time_steps, "_time_steps_", maf, "_maf.txt")
				run_simulation(maf, num_simulations, num_individuals, num_time_steps, interaction_effect_size, sdev, linear_effects_sdev, output_file)
			}
		}
	}
}
